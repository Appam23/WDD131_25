...
// 1. Function to generate a random number >= 0 and < num
function getRandomNum(num) {
    return Math.floor(Math.random() * num);
}

// 2. Function to return a random recipe using the imported recipes
function getRandomRecipe(recipes) {
    const randomIndex = getRandomNum(recipes.length);
    return recipes[randomIndex];
}

// 5. Template function to generate tags markup
function tagsTemplate(tags) {
    // Loop through the tags list and transform the strings to HTML <li> elements
    return tags.map(tag => `<li>${tag}</li>`).join('');
}

// 6. Template function to generate rating stars markup
...
function renderRecipes(recipeList) {
    ...
    if (recipeList.length === 0) {
        outputElement.innerHTML = `<h2>Featured Recipes</h2><p>No recipes found or unavailable.</p>`;
    } else {
        const html = recipeList.map(recipe => recipeTemplate(recipe)).join('');
        outputElement.innerHTML = `<h2>Featured Recipes</h2>${html}`;
    }
}
...
function filterRecipes(query) {
    ...
    return recipes
        .filter(recipe => {
            ...
            // Check ingredients
            if (recipe.recipeIngredient.find(ing => ing.toLowerCase().includes(query))) return true;
            return false;
        })
